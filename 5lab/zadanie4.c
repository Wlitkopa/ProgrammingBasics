...
void Timetable__init(struct Timetable* timetable, int hour, int minute, int duration, enum Day day) {
    timetable->lesson->term->hour = hour;
    timetable->lesson->term->minute = minute;
    timetable->lesson->term->duration = duration;
    timetable->lesson->term->day = day;
}

struct Timetable* Timetable__create(int hour, int minute, int duration, enum Day day){
    struct Timetable *timetable = malloc(sizeof (struct Term));
    Timetable__init(timetable, hour, minute, duration, day);
    return timetable;
}
...
char *Timetable__toString(struct Timetable *day[], int subcnt, int startminute, int starthour, int maxdur) {

    char *timetable = (char*)malloc(1000*sizeof (char));
    int ind = 0;
    int numless = 0;
    int newhour = 0;
    int newminute = 0;
    int curminute = startminute;
    int curhour = starthour;

    while ((maxdur + 60*((starthour)) + startminute) >= (60*curhour + curminute)){
        newminute = 90*numless;
        newhour = ((newminute - newminute%60)/60) + starthour;
        newminute = (newminute - newminute%60);

        if (curhour < 10){
            timetable[ind++] = '0' + curhour;
            timetable[ind++] = ':';
        }
        else{
            timetable[ind++] = '0' + (((curhour)-(curhour%10))/10);
            timetable[ind++] = '0' + (curhour%10);
            timetable[ind++] = ':';
        }
        if (curminute < 10){
            if (curminute == 0){
                timetable[ind++] = '0' + curminute;
            }
            timetable[ind++] = '0' + curminute;
        }
        else{
            timetable[ind++] = '0' + (((curminute)-(curminute%10))/10);
            timetable[ind++] = '0' + (curminute%10);
        }

        timetable[ind++] = '-';

        if (newhour < 10){
            timetable[ind++] = '0' + newhour;
            timetable[ind++] = ':';
        }
        else{
            timetable[ind++] = '0' + (((newhour)-(newhour%10))/10);
            timetable[ind++] = '0' + (newhour%10);
            timetable[ind++] = ':';
        }
        if (newminute < 10){
            if (newminute == 0){
                timetable[ind++] = '0' + newminute;
            }
            timetable[ind++] = '0' + newminute;
        }
        else{
            timetable[ind++] = '0' + (((newminute)-(newminute%10))/10);
            timetable[ind++] = '0' + (newminute%10);
        }

        for (int i = 0; i < subcnt; ++i) {

            int minlessontime = day[i]->lesson->term->hour*60 + day[i]->lesson->term->minute;
            int maxlessontime = day[i]->lesson->term->hour*60 + day[i]->lesson->term->minute + day[i]->lesson->term->duration;

            if (minlessontime < newhour*60 + newminute){
                for (int j = 0; j < strlen(day[i]->lesson->subject); ++j) {
                    timetable[ind++] = day[i]->lesson->subject[i];
                }
            }
        }
...
//    struct Lesson *lesson1 = Lesson__create(12, 50, 90, WED, "Podstawy programowania");
    struct Timetable *timetable[4];

    timetable[0] = Timetable__create(9, 30, 90, MON);
    timetable[1] = Timetable__create(11, 00, 90, MON);
    timetable[2] = Timetable__create(14, 00, 90, MON);
    timetable[3] = Timetable__create(18, 30, 90, MON);
...
//    printf("Po uwzględnieniu przesunięć: \n       %s\n", Lesson__toString(lesson1));
    ...
    char *timetable2 = (char*) malloc(1000*sizeof (char));
    timetable2 = Timetable__toString(timetable, 4, 0, 8, 119);
    printf("%s\n", timetable2);
